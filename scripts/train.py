...
Full training pipeline with:
  - Mixed precision (AMP)
  - ArcFace margin warmup (0 → 0.5 over 5 epochs)
...
Usage:
    ...
    python scripts/train.py --epochs 25 --batch-size 256 --lr 0.01
...
class TrainingConfig:
    ...
    batch_size: int = 256
    num_epochs: int = 25
    base_lr: float = 0.01  # Stable for single-GPU ArcFace training
...
def train_epoch(model, loader, optimizer, scaler, epoch, global_step, config,
                warmup_steps, total_steps, device, checkpoint_dir):
    """
    Train for one epoch.
    ...
    for batch_idx, (images, labels) in enumerate(pbar):
        ...
        # Accuracy on RAW cosine (before margin penalty)
        with torch.no_grad():
...
def validate(model, loader, config, device):
    ...
    This gives a true measure of how well the model separates identities,
    without the artificial difficulty introduced by the angular margin.
    """
    model.eval()
    total_loss = 0
    total_correct = 0
    ...
    for images, labels in loader:
        ...
        with autocast(device_type="cuda", enabled=config.use_amp):
            ...
            raw_logits = raw_cos * model.head.s
...
@torch.no_grad()
def validate_embeddings(model, loader, config, device, num_pairs=3000):
    """
    Embedding-based face verification (industry standard metric).

    Extracts backbone embeddings, generates positive/negative pairs,
    and computes verification accuracy, TAR@FAR=0.01, and AUC.
    """
    import random
...
def train(config: TrainingConfig):
    ...
    print(f"  AMP: {config.use_amp}, Grad clip: {config.grad_clip}")
    ...
    try:
        for epoch in range(start_epoch, config.num_epochs):
            ...
            train_loss, train_acc, global_step = train_epoch(
                ...
                device, config.checkpoint_dir,
            )

            # Classification validation
            has_val = val_loader is not None and len(val_dataset) > 0
            ...
            if has_val and (epoch + 1) % 2 == 0:
                try:
                    embed_metrics = validate_embeddings(model, val_loader, config, device)
                    embed_acc = embed_metrics["verification_accuracy"]
                    print(f"  Embed Acc: {embed_acc:.2f}%, TAR@FAR=1%: {embed_metrics['tar_at_far001']:.2f}%, AUC: {embed_metrics['auc']:.4f}")
                except Exception as e:
    ...
    if has_val:
        ...
        try:
            final = validate_embeddings(model, val_loader, config, device, num_pairs=5000)
